/* PixelSpot Samsung Tizen Player — playback engine */
...
const Player = (() => {
  ...
  function getTizenDeviceId() {
    try {
      if (typeof tizen !== 'undefined' && tizen.systeminfo) {
        return new Promise((resolve) => {
          tizen.systeminfo.getPropertyValue('DEVICE_ORIENTATION', () => {});
          tizen.systeminfo.getPropertyValue('BUILD', (build) => {
            resolve(`tizen-${build.model}-${build.manufacturer}`);
          }, () => resolve('tizen-unknown'));
        });
      }
    } catch (e) { /* */ }
    return Promise.resolve('tizen-' + Math.random().toString(36).substr(2, 8));
  }
    ...
    heartbeatTimer = setInterval(async () => {
      try {
        const res = await fetch(`${serverUrl}/api/v1/players/heartbeat`, {
          ...
          body: JSON.stringify({ currentContentId: null, playerVersion: 'samsung-tizen-1.0' }),
        });
        if (!res.ok) return;
  ...
  return { init, getTizenDeviceId, onPaired: (token) => { deviceToken = token; init(); } };
})();
