...
public class CreateCampaignWizardCommandValidator : AbstractValidator<CreateCampaignWizardCommand>
{
    public CreateCampaignWizardCommandValidator()
    {
        RuleFor(x => x.UserId)
            .NotEmpty()
        ...
        RuleFor(x => x.Request.Bookings)
            .NotEmpty()
            .WithMessage("At least one screen booking is required.");

        RuleForEach(x => x.Request.Bookings).ChildRules(booking =>
        {
            booking.RuleFor(b => b.ScreenId)
                .NotEmpty()
                .WithMessage("Screen ID is required for each booking.");

            booking.RuleFor(b => b.CreativeId)
                .NotEmpty()
                .WithMessage("Creative ID is required for each booking.");
        });
    }
