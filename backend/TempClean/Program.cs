...
foreach (var t in tables) { try { using var cmd = new NpgsqlCommand($"TRUNCATE TABLE \"{t}\" CASCADE", conn); cmd.ExecuteNonQuery(); Console.WriteLine($"Truncated {t}"); } catch { } }
Console.WriteLine("DB cleaned - ready for fresh registration test!");
